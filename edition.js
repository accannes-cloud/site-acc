...
(function () {
  ...
  function rendreEquipe() {
    ...
    // Bureau (clé "groupes") et Entraîneurs (clé "entraineurs_groupes")
    rendreGroupes(document.querySelector('[data-equipe-bureau]'), 'groupes');
    rendreGroupes(document.querySelector('[data-equipe-entraineurs-groupes]'), 'entraineurs_groupes');
  }

  // Rend une liste de groupes de personnes, modifiable (réutilisé bureau + entraîneurs)
  function rendreGroupes(box, cle) {
    if (!box || !Array.isArray(data[cle])) return;
    box.innerHTML = data[cle].map((g, gi) => {
    ...
    }).join('') + `<button class="acc-ajout" data-gadd style="background:#0F6E56;">➕ Ajouter un groupe</button>`;

    const L = data[cle];
    box.querySelectorAll('[data-gicone]').forEach(n => { const i=n.dataset.gicone; editable(n, () => modale(L[i].icone, v => { L[i].icone=v; n.textContent=v; marquerModifie(); })); });
    box.querySelectorAll('[data-gtitre]').forEach(n => { const i=n.dataset.gtitre; editable(n, () => modale(L[i].titre, v => { L[i].titre=v; n.textContent=v; marquerModifie(); })); });
    box.querySelectorAll('[data-gsous]').forEach(n => { const i=n.dataset.gsous; editable(n, () => modale(L[i].sous_titre, v => { L[i].sous_titre=v; n.textContent=v; marquerModifie(); })); });
    box.querySelectorAll('[data-mnom]').forEach(n => { const [gi,mi]=n.dataset.mnom.split('-'); editable(n, () => modale(L[gi].membres[mi].nom, v => { L[gi].membres[mi].nom=v; n.textContent=v; marquerModifie(); })); });
    box.querySelectorAll('[data-mposte]').forEach(n => { const [gi,mi]=n.dataset.mposte.split('-'); editable(n, () => modale(L[gi].membres[mi].poste, v => { L[gi].membres[mi].poste=v; n.textContent=v; marquerModifie(); })); });
    box.querySelectorAll('[data-mrole]').forEach(n => { const [gi,mi]=n.dataset.mrole.split('-'); editable(n, () => modale(L[gi].membres[mi].role, v => { L[gi].membres[mi].role=v; n.textContent=v; marquerModifie(); })); });
    box.querySelectorAll('[data-mphoto]').forEach(n => {
      const [gi,mi]=n.dataset.mphoto.split('-');
      boutonPhoto(n, () => choisirImage(c => { L[gi].membres[mi].photo=c; marquerModifie(); rendreEquipe(); }, 500), '📷');
      if (L[gi].membres[mi].photo) badgeSuppr(n, () => { L[gi].membres[mi].photo=''; marquerModifie(); rendreEquipe(); });
    });
    box.querySelectorAll('[data-mcard]').forEach(n => {
      const [gi,mi]=n.dataset.mcard.split('-');
      const del = el('<button class="acc-suppr-badge">✕</button>');
      del.onclick = (e) => { e.preventDefault(); e.stopPropagation(); if(confirm('Supprimer cette personne ?')){ L[gi].membres.splice(mi,1); marquerModifie(); rendreEquipe(); } };
      if (getComputedStyle(n).position === 'static') n.style.position = 'relative';
      n.appendChild(del);
    });
    box.querySelectorAll('[data-maddmembre]').forEach(n => {
      const gi = n.dataset.maddmembre;
      n.onclick = () => { L[gi].membres.push({nom:'Nouvelle personne', poste:'', role:'', photo:''}); marquerModifie(); rendreEquipe(); };
    });
    const gadd = box.querySelector('[data-gadd]');
    if (gadd) gadd.onclick = () => { L.push({icone:'🏃', titre:'NOUVEAU GROUPE', sous_titre:'Description', membres:[]}); marquerModifie(); rendreEquipe(); };
  }
